SIH_function<-function(dispersal=0.001,species=9,patches=30,eff_vary=F,eff_values=NA){
  ...
  phylo<-as.phylo(hclust(daisy(cbind(eOptimum,eff_vary)),method ="ward.D"))
  plot(phylo,show.tip.label = F)
  tiplabels(pch=22,bg=heat.colors(species)[1:species])
  ...
  return(list(Prod=Prod,Abund=Abund,phylo=phylo))
}
